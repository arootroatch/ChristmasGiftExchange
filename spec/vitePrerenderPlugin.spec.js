...
<body></body>
    ...
    it("generates og:url as root for index.html", async () => {
      ...
      const result = await plugin.transformIndexHtml(html, {path: "/index.html"});
      expect(result).toContain('og:url" content="https://gift-exchange-generator.com/"');
    });

    it("derives /wishlist/edit from pages/wishlist/edit path", async () => {
      ...
      const result = await plugin.transformIndexHtml(html, {path: "/pages/wishlist/edit/index.html"});
      expect(result).toContain('og:url" content="https://gift-exchange-generator.com/wishlist/edit"');
    });

    it("includes og:image and og:type", async () => {
      const result = await plugin.transformIndexHtml(pageHtml, {path: "/pages/reuse/index.html"});
    ...
    it("injects OG tags before </head>", async () => {
      const result = await plugin.transformIndexHtml(pageHtml, {path: "/pages/reuse/index.html"});
    ...
    it("injects canonical link with correct URL for subpage", async () => {
      const result = await plugin.transformIndexHtml(pageHtml, {path: "/pages/reuse/index.html"});
      expect(result).toContain('<link rel="canonical" href="https://gift-exchange-generator.com/reuse">');
    });

    it("injects canonical link with root URL for index.html", async () => {
      const html = `<html><head><meta charset="UTF-8"><title>Gift Exchange Generator</title><meta name="description" content="Draw names."></head><body></body></html>`;
      const result = await plugin.transformIndexHtml(html, {path: "/index.html"});
...
<body></body>
    ...
    it("injects favicon links into pages that lack them", async () => {
      const result = await plugin.transformIndexHtml(bareHtml, {path: "/pages/wishlist/edit/index.html"});
    ...
    it("does not duplicate favicons when already present", async () => {
      ...
      const result = await plugin.transformIndexHtml(htmlWithFavicons, {path: "/index.html"});
      const count = (result.match(/rel="apple-touch-icon"/g) || []).length;
      expect(count).toBe(1);
    });

    it("injects favicons after meta charset", async () => {
      const result = await plugin.transformIndexHtml(bareHtml, {path: "/pages/reuse/index.html"});
