...
export const handler = async (event) => {
  ...
  try {
    ...
    // Find all exchanges this user participated in
    ...
    // Resolve participant names and house info for each exchange
    const results = await Promise.all(exchanges.map(async (exchange) => {
      ...
      const userIdToName = {};
      participantUsers.forEach(u => { userIdToName[u._id.toString()] = u.name; });
