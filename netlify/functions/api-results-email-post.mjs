...
async function resolveAssignmentsFromExchange(exchangeId, user) {
    ...
    if (!exchange.organizer || !exchange.organizer.equals(user._id)) {
        return {error: forbidden("Only the organizer can send results emails")};
    }
...
export const handler = apiHandler("POST", async (event) => {
    ...
    if ("exchangeId" in data) {
        const result = await resolveAssignmentsFromExchange(data.exchangeId, user);
